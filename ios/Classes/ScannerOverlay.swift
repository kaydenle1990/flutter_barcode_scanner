...
class ScannerOverlay: UIView {
    ...
    func startAnimating() {
        ...
        flash.fromValue = NSValue(cgPoint: CGPoint(x: scanRect.origin.x + scanRect.width / 2, y: scanRect.origin.y))
        flash.toValue =  NSValue(cgPoint: CGPoint(x: scanRect.origin.x + scanRect.width / 2, y: scanRect.origin.y + scanRect.height))
        flash.duration = 2
